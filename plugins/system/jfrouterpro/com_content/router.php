...
function JFContentBuildRoute(&$query)
{
	$db =& JFactory::getDBO();
	$pfunc = $db->_profile();
	...
	if ($jfm->getCfg("transcaching",1)){
		...
		$result =  $cache->get( "JFContentBuildRouteCached", array($query, $language,$forceTranslation));
	}
	else {
		$result = JFContentBuildRouteCached($query, $language,$forceTranslation);
	}
	$query = $result["query"];
	$db->_profile($pfunc);
	return $result["val"];
...
function JFContentBuildRouteCached(&$query, $jfLang,$forceTranslation)
{
	$result = array();
	$result["query"] =& $query;
	$result["val"]="";
		...
		if(isset($query['id'])) {
			if (isset($query['view']) && $query['view']=="category" && !isset($query['catid'])){
				...
				if (count($parts)>=2){
					$cat = intval($parts[0]);
					$sql = "SELECT value FROM #__jf_content as jfc LEFT JOIN #__languages as lang ON jfc.language_id=lang.id ";
					$sql .= "WHERE reference_table='content' AND reference_field='alias' AND reference_id=$cat AND lang.code='".$jfLang."'";
					$db->setQuery($sql);
					$cattrans = $db->loadObject(false);
					if (!is_null($cattrans) && isset($cattrans->value)){
						$query['id']=trim($cat.":".$cattrans->value);
		...
		if(isset($query['id'])) {
			if (isset($query['view']) && $query['view']=="category" && !isset($query['catid'])){
				...
				if (count($parts)>=2){
					$cat = intval($parts[0]);
					$sql = "SELECT a.alias as value ";
					$sql .= "FROM #__content AS a WHERE id=$cat";
					$db->setQuery($sql);
					$cattrans = $db->loadObject(false);
					if (!is_null($cattrans) && isset($cattrans->value)){
						$query['id']=trim($cat.":".$cattrans->value);
...
function JFContentParseRoute($segments)
{
	// Use the component routing handler if it exists

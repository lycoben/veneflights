	...
	function selectList( &$arr, $tag_name, $tag_attribs, $key, $text, $selected, $required = 0 ) {
		...
		for ( $i = 0, $n = count( $arr ); $i < $n; $i++ ) {
			...
			if ( is_array( $arr[$i]->$key ) ) {
				...
				// &nbsp; of userGroups: cannot do this: htmlspecialchars( getLangDefinition( $t );
				$html				.=	str_replace( '&amp;nbsp;', '&nbsp;', htmlspecialchars( getLangDefinition( $t ) ) );
				$html				.=	"</option>";
	...
	*/
	...
	function _addHeadTag() {
		...
		if ( ! $added[$this->calendarType] ) {
			...
			if ( $this->calendarType == 1 ) {
				addCbHeadTag( $this->ui, '<link rel="stylesheet" type="text/css" media="all" href="'.selectTemplate( $this->ui ).'calendar.css" title="win2k-cold-1" />'
	...
	*/	
	...
	function getEditTabs( &$user, $postdata = null, $formatting = 'table', $reason = 'profile', $tabbed = true ) {
		...
		if ( $initFieldsToDefault ) {
			$fields							=	$this->_getTabFieldsDb( null, $user, ( $reason == 'profile' ? 'edit' : $reason ), null, true );
			if ( is_array( $fields ) ) {
		...
		if ( $prefetched !== null ) {
			if ( $tabid ) {
				...
				if (isset( $fieldsByName[$fieldName] ) ) {
					$fields		=	array( $fieldsByName[$fieldName] );
				} else {
					$fields		=	array();
				}
			} else {
	...
	function _getEditTabContents( &$tab, &$user, $formatting = 'table', $reason = 'profile', $prefetchFields = true ) {
		...
		$fields					=	$this->_getTabFieldsDb( $tabid, $user, ( $reason == 'profile' ? 'edit' : $reason ), null, $prefetchFields );
		if ( count( $fields ) > 0 ) {
			if ( $reason == 'profile' ) {
...
*/
	...
	function saveTabsContents( &$user, &$postdata, $reason ) {
		...
		$fields					=	$this->_getTabFieldsDb( null, $user, ( $reason == 'profile' ? 'edit' : $reason ), null, false );
		$result					=	true;
	...
	*/	
	...
	function fieldCall( $fieldName, &$user, &$postdata, $reason ) {
		global $_CB_framework, $_PLUGINS;
...
class cbNotification {
	...
	function sendFromSystem( $toid, $sub, $message, $replaceVariables = true, $mode = 0, $cc = null, $bcc = null, $attachment = null ) {
		global $_CB_framework, $_CB_database, $ueConfig;

...
<?php
	...
	function editList( &$row, $lists, $fields, $option, $tabid, $paramsEditorHtml ) {
		...
		$_CB_database->setQuery( "SELECT f.fieldid, f.title, f.name"
			...
			. "\n WHERE f.fieldid IN (" . implode( ',', array_diff( $fields, $fieldids ) ) . ')'
			. "\n  AND p.published = 1"
		);
...
?>
	...
	?></td>
	  ...
	  ?></td>
      <td width="10%"><a href="javascript: void(0);" onClick="return listItemTask('cb<?php echo $i;?>
','<?php echo $task;?>')"><img src="<?php echo $imgpath.$img;?>" width="16" height="16" border="0" alt="" /></a></td>
      <td width="10%"><a href="javascript: void(0);" onClick="return listItemTask('cb<?php echo $i;?>
','<?php echo $task2;?>')"><img src="<?php echo $imgpath.$img2;?>" width="16" height="16" border="0" alt="" /><?php echo $text2;?></a></td>
      <td width="10%"><a href="javascript: void(0);" onClick="return listItemTask('cb<?php echo $i;?>
','<?php echo $task4;?>')"><img src="<?php echo $imgpath.$img4;?>" width="16" height="16" border="0" alt="" /></a></td>
      <td width="10%"><?php
      if ( $row->tablecolumns != '' && ! in_array( $row->type, array( 'password', 'userparams' ) ) ) {
		?><a href="javascript: void(0);" onClick="return listItemTask('cb<?php echo $i;?>
','<?php echo $task5;?>')"><img src="<?php echo $imgpath.$img5;?>" width="16" height="16" border="0" alt="" /></a><?php
      } else {
...
//		$('#fieldValuesBody').append('<tr><td><input id=\"vNames'+i+'\" name=\"vNames[' + i + ']\" /></td></tr>');
  ...
  $('#name').change(function() { if ( ! $(this).attr('disabled') ) { prep4SQL(this); } } ).change();
...
<?php
		...
		<tr>
			...
			<td width="20%"><?php
			if ( $row->type == 'editorta' ) {
				echo $_CB_framework->displayCmsEditor( 'default', $row->default, 600, 200, 50, 7 );
			} else {
				?><input type="text" name="default" mosLabel="<?php echo CBTxt::T( "Default value" ); ?>" class="inputbox" value="<?php echo htmlspecialchars( $row->default ); ?>" /><?php
			}
			?></td>
			<td>&nbsp;</td>
		</tr>
...
<script  type="text/javascript" src="<?php echo $_CB_framework->getCfg( 'live_site' );?>/includes/js/overlib_mini.js"></script>
<?php 
	if($row->fieldid > 0) {
		print "<script type=\"text/javascript\"> document.adminForm.name.readOnly=true; </script>";
		print "<script type=\"text/javascript\"> document.adminForm.name.disabled=true; </script>";
...
<script type="text/javascript"><!--//--><![CDATA[//><!--
    ...
    function cbCheckVersion() {
    	...
    	CBmakeHttpRequest('<?php 
    		echo "index3.php?option=com_comprofiler&task=latestVersion&no_html=1&format=raw";

...
var MediaManager = {
    ...
    initialize: function()
    {
        ...
        this.tree = new MooTreeControl({ div: 'media-tree_tree', mode: 'folders', grid: true, theme: 'components/com_yos_media/assets/mootree.gif', onClick:
                function(node){
                    target = $chk(node.data.target) ? node.data.target : '_self';
                    window.frames[target].location.href = node.data.url;
                }
            },{ text: 'Media', open: true, data: { url: 'index.php?option=com_yos_media&view=mediaList&tmpl=component', target: 'folderframe'}});
    ...
    setViewType: function(type)
    {
        ...
        viewstyle = type;
        var folder = this.getFolder();
        this._setFrameUrl('index.php?option=com_yos_media&view=mediaList&tmpl=component&folder='+folder+'&layout='+type);
    },

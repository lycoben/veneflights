 ...
 * @subpackage	Components
 */
class TranslatorModelMenus extends JModel
{
	/**
	 * menus data array
	 *
...
/**
	...
	function __construct()
	{
		...
		$filter_state		= $mainframe->getUserStateFromRequest( $option.'.menu.filter_state',		'filter_state',		'',	'word' );	
		$search 			= $mainframe->getUserStateFromRequest( $option.'.menu.search', 			'search', 			'',				'string' );		
		$search 			= JString::strtolower( $search );
		
		$filter_language	= $mainframe->getUserStateFromRequest($option.'.menu.filter_language', 'filter_language', '', 'string');
		
		$filter_status		= $mainframe->getUserStateFromRequest($option.'.menu.filter_status', 'filter_status', -1, 'int');
	
		$limit				= $mainframe->getUserStateFromRequest($option.'.menu.limit', 'limit', $mainframe->getCfg('list_limit'), 'int');
		$limitstart			= $mainframe->getUserStateFromRequest($option.'.menu.limitstart', 'limitstart', 0, 'int');
	...
	function _buildQuery($total = false){
		...
		if (!$total) {
			$query = "
			SELECT JM.`id`, JM.`name` AS jmname, JFC.`language_id`, JL.`name` AS jlname,
			JFC.value AS 'translation',
			JFC.modified AS 'modified',
			JFC.modified_by AS 'modified_by',	
			JFC.published AS 'published',
			(SELECT md5(JM1.name) = JMC1.original_value FROM `#__menu` AS JM1
 			LEFT JOIN `#__jf_content` AS JMC1 ON JMC1.reference_id = JM1.id
		...
		$query .= "
			FROM `#__menu` AS JM
			LEFT JOIN `#__jf_content` AS JFC ON JM.id = JFC.reference_id AND JFC.reference_field = 'name' AND JFC.reference_table = 'menu' $leftJoinLanguage";
	...
	{		
		...
		$where[] = 'JM.`published` >= 0';
		
		if($search) {
			$where[]='(LOWER(JM.name) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false ).')';
		}

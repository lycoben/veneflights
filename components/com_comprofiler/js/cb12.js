...
function cbParamChange() {
	...
	for (i=0;i<fieldsToSet.length;i++) {
		if ( cbGetDisplayStyle( document.getElementById( cbHideFields[fieldsToSet[i]][0] ) ) != 'none' ) {
			for (j=0;j<cbHideFields[fieldsToSet[i]][5].length;j++) {
				nameValue  = cbHideFields[fieldsToSet[i]][5][j].split('=',3);
				if ( cbGetDisplayStyle( document.getElementById( cbHideFields[fieldsToSet[i]][4][0] /* nameValue[0] */ ) ) != 'none' ) {
	...
	for (i=0;i<fieldsToRestore.length;i++) {		// TBD:Opera doesn't restore correctly with radio choice
		if ( cbGetDisplayStyle( document.getElementById( cbHideFields[fieldsToRestore[i]][0] ) ) != 'none' ) {
			for (j=0;j<cbHideFields[fieldsToRestore[i]][5].length;j++) {
				nameValue  = cbHideFields[fieldsToRestore[i]][5][j].split('=',3);
				if ( cbGetDisplayStyle( document.getElementById( cbHideFields[fieldsToRestore[i]][4][0] /* nameValue[0] */ ) ) == 'none' ) {
					inputToSet = document.getElementById( nameValue[1] );

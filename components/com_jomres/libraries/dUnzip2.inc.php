				...
				foreach($folders as $folder){
					...
					if(!is_dir($targetDir.JRDS.$str)){
						if ($str!="." && $str!=".." )
							...
							if (!recursive_mk_dir($targetDir.JRDS.$str))
								$this->debugMsg(1, "Failed to create: ".$targetDir.JRDS.$str);
							if($applyChmod)
...
function recursive_mk_dir($path) 
  ...
  while(!@is_dir(dirname(end($folder_path)))
	...
	while($parent_folder_path = array_pop($folder_path)) 
		{
		if(!@mkdir($parent_folder_path, umask() )) 
			{

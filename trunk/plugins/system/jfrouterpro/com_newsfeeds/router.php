...
function JFNewsfeedsBuildRoute(&$query)
{
	$db =& JFactory::getDBO();
	$pfunc = $db->_profile();
	...
	if ($jfm->getCfg("transcaching",1)){
		...
		$result =  $cache->get( "JFNewsfeedsBuildRouteCached", array($query, $language,$forceTranslation));
	}
	else {
		$result = JFNewsfeedsBuildRouteCached($query, $language,$forceTranslation);
	}
	$query = $result["query"];
	$db->_profile($pfunc);
	return $result["val"];
...
function JFNewsfeedsBuildRouteCached(&$query, $jfLang,$forceTranslation)
{
	$result = array();
	$result["query"] =& $query;
	$result["val"]="";
	...
	if ($jfLang	!= $siteLang){
		...
		if(isset($query['id'])) {
			if (isset($query['view']) && $query['view']=="category" && !isset($query['catid'])){
				...
				if (count($parts)>=2){
					$cat = intval($parts[0]);
					$sql = "SELECT value FROM #__jf_content as jfc LEFT JOIN #__languages as lang ON jfc.language_id=lang.id ";
					$sql .= "WHERE reference_table='newsfeeds' AND reference_field='alias' AND reference_id=$cat AND lang.code='".$jfLang."'";
					$db->setQuery($sql);
					$cattrans = $db->loadObject(false);
					if (!is_null($cattrans) && isset($cattrans->value)){
						$query['id']=trim($cat.":".$cattrans->value);
					...
					// Newsfeeds in Joomla 1.5.3 uses a - instead of a : for some reason!
					$parts = explode("-",$query['id'],2);
					if (count($parts)>=2){
						$cat = intval($parts[0]);
						if ($cat>0){
							$sql = "SELECT value FROM #__jf_content as jfc LEFT JOIN #__languages as lang ON jfc.language_id=lang.id ";
		...
		if(isset($query['id'])) {
			...
			if (count($parts)>=2){
				$cat = intval($parts[0]);
				$sql = "SELECT a.alias as value ";
				$sql .= "FROM #__newsfeeds AS a WHERE catid=$cat";
				$db->setQuery($sql);
				$cattrans = $db->loadObject(false);
				if (!is_null($cattrans) && isset($cattrans->value)){
					$query['id']=trim($cat.":".$cattrans->value);
...
function JFNewsfeedsParseRoute($segments)
{
	// Use the component routing handler if it exists

...
$_PLUGINS->registerFunction( 'onBeforeUserProfileDisplay', 'prepareMenu','getMenuTab' );
$_PLUGINS->registerFunction( 'onBeforeUserProfileDisplay', 'prepareStatus','getStatusTab' );
...
// class cbMenuBar extends cbSuckerMenuHandler {
	function cbMenuBar( /* $ui=1 */ ) {
	$menuParent = get_parent_class($this);
	$this->$menuParent();
	}
	function outputScripts( $ui = 1 ) {
		global $_CB_framework;
		addCbHeadTag( $ui, "<script type=\"text/javascript\" src=\"".$_CB_framework->getCfg( 'live_site' ) . '/components/com_comprofiler/js/menubest' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . ".js\"></script>\n" );
		return null;
	}
	function languageTranslate($string) {
...
class getMenuTab  extends cbTabHandler {
	...
	*/
	...
	/**
	* Method is called before the user profile is displayed (onBeforeUserProfileDisplay event)
	* @param 	object	  	holds the core mambo user data
	* @param 	int			user interface: ui=1 front, ui=2 backend
	* @param 	boolean		true if displayed user is moderator
	* @param 	boolean		true if curent user is moderator
	*/
	function prepareMenu($user, $ui, $cbUserIsModerator, $cbMyIsModerator) {
		$this->ui = $ui;
		$this->cbUserIsModerator = $cbUserIsModerator;
		$this->cbMyIsModerator = $cbMyIsModerator;
		$params=$this->params;
		...
		return $this->menuBar->outputScripts(1);
	}
	/**
...
class getStatusTab extends cbTabHandler {
	...
	* @param 	boolean		true if curent user is moderator
	*/
	function prepareStatus($user, $ui, $cbUserIsModerator, $cbMyIsModerator) {
		$this->ui = $ui;
		$this->cbUserIsModerator = $cbUserIsModerator;
		$this->cbMyIsModerator = $cbMyIsModerator;
		$params=$this->params;

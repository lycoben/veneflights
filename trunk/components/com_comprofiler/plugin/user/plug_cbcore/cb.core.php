...
class CBfield_textarea extends CBfield_text {
	...
	function getField( &$field, &$user, $output, $reason ) {
		switch ( $output ) {
			case 'html':
			case 'rss':
				return str_replace( "\n", '<br />', parent::getField( $field, $user, $output, $reason ) );
			default:
				return parent::getField( $field, $user, $output, $reason );
				break;
		}
	}
}
...
class CBfield_predefined extends CBfield_text {
	...
	function getField( &$field, &$user, $output, $reason ) {
		...
		switch ( $output ) {
			case 'html':
			case 'rss':
				if ( ( $field->type == 'predefined' ) && ( $ueConfig['allow_profilelink'] == 1 ) && ( $reason != 'profile' ) ) {
...
//					if ( ( $ueConfig["usernameedit"] == 1 ) || ( $user->username == '' ) || ( $_CB_framework->getUi() == 2 ) ) {
					if ( ! ( ( $ueConfig['usernameedit'] == 0 ) && ( $reason == 'profile' ) && ( $_CB_framework->getUi() == 1 ) ) ) {
				...
				if ( $reason == 'search' ) {
					$html					=	$this->_fieldSearchModeHtml( $field, $user, $html, 'text' );
				}
				return $html;
				break;
						...
						if ( $_CB_database->loadObject( $dataObj ) ) {
							if ( $dataObj->result ) {
								...
								if ( $function == 'testexists' ) {
									$html	=	( '<span class="cb_result_ok">' . sprintf( _UE_USERNAME_EXISTS_ON_SITE, htmlspecialchars( $username ) ) . '</span>' );
								} else {
									$html	=	( '<span class="cb_result_error">' . sprintf( _UE_USERNAME_ALREADY_EXISTS, htmlspecialchars( $username ) ) . '</span>' );
								}
							} else {
								if ( $function == 'testexists' ) {
									$html	=	( '<span class="cb_result_error">' . sprintf( _UE_USERNAME_DOES_NOT_EXISTS_ON_SITE, htmlspecialchars( $username ) ) . '</span>' );
								} else {
									if ( $reason == 'register' ) {
										$html	=	( '<span class="cb_result_ok">' . sprintf( _UE_USERNAME_DOESNT_EXISTS, htmlspecialchars( $username ) ) . '</span>' );
									} else {
										$html	=	( '<span class="cb_result_ok">' . sprintf( _UE_USERNAME_FREE_OK_TO_PROCEED, htmlspecialchars( $username ) ) . '</span>' );
									}
								}
							}
						} else {
							$html			=	( '<span class="cb_result_error">' . _UE_SEARCH_ERROR . ' !' . '</span>' );
						}
					}
				} else {
					if ( $user && ( $user->id == $_CB_framework->myId() ) ) {
						$html			=	( '<span class="cb_result_ok">' . sprintf( _UE_THIS_IS_YOUR_USERNAME, htmlspecialchars( $username ) ) . '</span>' );
					} else {
						$html			=	( '<span class="cb_result_ok">' . sprintf( _UE_THIS_IS_USERS_USERNAME, htmlspecialchars( $username ) ) . '</span>' );
					}
				}
			} else {
				$html					=	_UE_NOT_AUTHORIZED;
			}
		}
		return $html;
	}
		...
		switch ( $field->name ) {
			case 'username':
				if ( ! ( ( $ueConfig['usernameedit'] == 0 ) && ( $reason == 'profile' ) && ( $_CB_framework->getUi() == 1 ) ) ) {
					...
					if ( strlen( $username ) < $fieldMinLength ) {
				...
				if ( $value !== null ) {
					...
					if ( $field->name !== 'name' ) {
						...
						switch ( $ueConfig['name_style'] ) {
							case 2:
								if ( $user->firstname ) {
									$nameArr[]	=	$user->firstname;
								}
								if ( $user->lastname ) {
									$nameArr[]	=	$user->lastname;
								}
								break;
							case 3:
								if ( $user->firstname ) {
...
class CBfield_password extends CBfield_text {
	...
	function getFieldRow( &$field, &$user, $output, $formatting, $reason ) {
		...
		if ( $output == 'htmledit' ) {
			...
			if ( ( $reason != 'register' ) || ( ! isset( $ueConfig['emailpass'] ) ) || ( $ueConfig['emailpass'] == "0")  || ( $ueConfig['reg_confirmation'] == "1") || ( $field->name != 'password' ) ) {
				...
				$results						=	parent::getFieldRow( $field, $user, $output, $formatting, $reason );
		...
		switch ( $output ) {
			case 'htmledit':
				...
				if ( ( $reason != 'register' ) || ( ! isset( $ueConfig['emailpass'] ) )|| ( $ueConfig['emailpass'] == "0")  || ( $ueConfig['reg_confirmation'] == "1") || ( $field->name != 'password' ) ) {
					...
					$req							=	$field->required;
					if ( ( $reason == 'profile' ) && in_array( $field->name, array( 'password', 'password__verify' ) ) ) {
						$field->required			=	0;
					}
					$html							=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', $field->type, $value, '' );
					$field->required				=	$req;
	...
	function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
		...
		foreach ( $field->getTableColumns() as $col ) {
			...
			if ( ( $reason == 'profile' ) && ( $user->id != 0 ) && ( $user->$col || ( $field->name == 'password' ) ) ) {
			...
			if ( $value ) {
				if ( strlen( $value ) < $fieldMinLength ) {
	 ...
	 * @return boolean                          True: All fields have validated, False: Some fields didn't validate
	 */
	function bindSearchCriteria( &$field, &$user, &$postdata, $reason ) {
		return array();
	}
}
		...
		switch ( $output ) {
			...
			case 'rss':
				...
				for( $i = 0, $n = count( $chosen ); $i < $n; $i++ ) {
	   				$vals[]		=	htmlspecialchars( getLangDefinition( $chosen[$i] ) );
				}
				$displayStyle	=	$field->params->get( 'field_display_style' );
				switch ( $displayStyle ) {
					case 1:
					case 2:
						$listType		=	( $displayStyle == 1 ? 'ul' : 'ol' );
						if ( count( $vals ) > 0 ) {
...
//					$html			=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'multicheckbox', $value, '', $allValues );
					...
					$html			=	$this->_fieldSearchModeHtml( $field, $user, $html, ( strpos( $field->type, 'multi' ) === 0 ? 'multiplechoice' : 'singlechoice' ), $jqueryclass );
				} else {
			...
			case 'xml':
				$chosen			=	$this->_explodeCBvalues( $value );
				for( $i = 0, $n = count( $chosen ); $i < $n; $i++ ) {
	   				$vals[]		=	'<value value="' . htmlspecialchars( $chosen[$i] ) . '">' . htmlspecialchars( getLangDefinition( $chosen[$i] ) ) . '</value>';
				}
				return '<' . $field->name . '>' . implode( '', $vals ) . '</' . $field->name . '>';
				break;

			case 'json':
				$chosen			=	$this->_explodeCBvalues( $value );
				for( $i = 0, $n = count( $chosen ); $i < $n; $i++ ) {
	   				$vals[]		=	"'" . addslashes( getLangDefinition( $chosen[$i] ) ) . "'";
				}
				return "'" . $field->name . "' : { " .  implode( ', ', $vals ) . " }";
				break;
			...
			case 'php':
				...
				return array( $field->name => $chosen );
				break;

			case 'csv':
				$chosen			=	$this->_explodeCBvalues( $value );
				for( $i = 0, $n = count( $chosen ); $i < $n; $i++ ) {
	   				$vals[]		=	getLangDefinition( $chosen[$i] );
				}
				return '"' . implode( ', ', $vals ) . '"';
				break;
			...
			foreach ( $field->getTableColumns() as $col ) {
				...
				if ( is_array( $value ) ) {
					if ( count( $value ) > 0 ) {
						...
						foreach ( $value as $k => $v ) {
							if ( ( count( $value ) == 1 ) && ( $v === '' ) ) {
								$value		=	'';		// None Preferences: checked: search for nothing selected
								break;
							}
							// revert escaping of cbGetParam:
							$v				=	stripslashes( $v );
		...
		switch ( $output ) {
			...
			case 'htmledit':
				if ( $reason == 'search' ) {
					...
					if ( $reason == 'search' ) {
						$html			=	$this->_fieldSearchModeHtml( $field, $user, $html, 'none' );
					}
					return $html;
				} else {
				$checked		=	'';
...
class CBfield_integer extends CBfield_text {
		...
		switch ( $output ) {
			case 'htmledit':
				if ( $reason == 'search' ) {
					...
					$ret			=	$this->_fieldSearchRangeModeHtml( $field, $user, $output, $reason, $value, $minHtml, $maxHtml );
			...
			case 'csv':
			default:
				$ret				=	parent::getField( $field, $user, $output, $reason );
				if ( ( $ret !== '' ) && ( $ret !== null ) ) {
					$ret			=	(int) $ret;
				}
				break;
	...
	function validate( &$field, &$user, $columnName, &$value, &$postdata, $reason ) {
		...
		if ( $validated && ( $value !== '' ) && ( $value !== null ) ) {		// empty values (e.g. non-mandatory) are treated in the parent validation.
			...
			if ( $validated ) {
				// check range:
				$min				=	$field->params->get( 'integer_minimum', '0' );
				$max				=	$field->params->get( 'integer_maximum', '1000000' );
				if ( $max < $min ) {
...
class CBfield_date extends cbFieldHandler {
		...
		switch ( $output ) {
			...
			case 'htmledit':
				...
				if ( $reason == 'search' ) {
					...
					$html	=	$this->_fieldSearchRangeModeHtml( $field, $user, $output, $reason, $value, $minHtml, $maxHtml );
					return $html;
	...
	function _yearSetting( $setParam, $outputMode ) {
		...
		if ( $outputMode == 1 ) {
			if ( $offset === null ) {
				$offset				=	$fullYear - date( 'Y', time() +  $_CB_framework->getCfg( 'offset' ) );
			}
			return -$offset;
		} else {
			if ( $offset !== null ) {
				$fullYear			=	date( 'Y', time() +  $_CB_framework->getCfg( 'offset' ) ) + $offset;
			}
			return $fullYear;
		}
	}
		...
		if ( $searchMode ) {
			foreach ( $field->getTableColumns() as $col ) {
				...
				if ( $search_by == 1 ) {
					...
					$yMin				=	$y - $maxValIn;	// yes, crossed: the more years back, the smaller the date...	
					$yMax				=	$y - $minValIn;
	...
	function getField( &$field, &$user, $output, $reason ) {
		...
		switch ( $output ) {
			...
			case 'htmledit':
				...
				if ( $reason == 'search' ) {
					$oReturn			=	$this->_fieldSearchModeHtml( $field, $user, $oReturn, 'text' );
				}
				break;
			...
			case 'csv':
			default:
				$oReturn				=	parent::getField( $field, $user, $output, $reason );
				break;
		}
		return $oReturn;
	}
			...
			&& ( $reason != 'search' ) )
		{
			...
			if ( $function == 'checkvalue' ) {
				...
				if ( 	(	( isset( $ueConfig['reg_email_checker'] ) ? ( $ueConfig['reg_email_checker'] > 1 ) : false ) 
							&& ( ( $reason == 'register' ) || ( ( $reason == 'profile') && $user && ( $emailISO != $user->email ) ) ) )
						|| ( $_CB_framework->getUi() == 2 ) )
				{
					if ( $_CB_database->isDbCollationCaseInsensitive() ) {
					...
					if ( $_CB_database->loadObject( $dataObj ) ) {
						if ( $function == 'testexists' ) {
							if ( $dataObj->result ) {
								return '<span class="cb_result_ok">' . sprintf( _UE_EMAIL_EXISTS_ON_SITE, htmlspecialchars( $email ) ) . "</span>";
							} else {
								return '<span class="cb_result_error">' . sprintf( _UE_EMAIL_DOES_NOT_EXISTS_ON_SITE, htmlspecialchars( $email ) ) . "</span>";
							}
						} else {
							if ( $dataObj->result ) {
								return '<span class="cb_result_error">' . sprintf( _UE_EMAIL_ALREADY_REGISTERED, htmlspecialchars( $email ) ) . "</span>";
							}
						}
					}
				}
				...
				switch ( $checkResult ) {
					case -2:
						return '<span class="cb_result_error">' . sprintf( _UE_EMAIL_NOVALID, htmlspecialchars( $email ) ) . "</span>";
						break;
					case -1:
						return '<span class="cb_result_warning">' . sprintf( _UE_EMAIL_COULD_NOT_CHECK, htmlspecialchars( $email ) ) . "</span>";
						break;
					case 0:
						if ( $ueConfig['reg_confirmation'] == 0 ) {
							return '<span class="cb_result_error">' . sprintf( _UE_EMAIL_INCORRECT_CHECK, htmlspecialchars( $email ) ) . "</span>";
						} else {
							return '<span class="cb_result_error">' . sprintf( _UE_EMAIL_INCORRECT_CHECK_NEEDED, htmlspecialchars( $email ) ) . "</span>";
						}
						break;
					case 1:
						return '<span class="cb_result_ok">' . sprintf( _UE_EMAIL_VERIFIED, htmlspecialchars( $email ) ) . "</span>";
						break;
					default:
...
class CBfield_pm extends cbFieldHandler {
		...
		if ( count( $resultArray ) > 0) {
			switch ( $output ) {
				...
				default:
					...
					$oReturn					=	$this->_arrayToFormat( $retArray, $output );
					break;
			}
		}
		return $oReturn;
	...
	function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
		...
		return true;				// nothing to do, PM fields don't save :-)
	}
	/**
	 * Finder:
	 * Prepares field data for saving to database (safe transfer from $postdata to $user)
			...
			switch ( $output ) {
				...
				case 'htmledit':
					if ( $reason == 'search' ) {
						...
						$value			=	$user->$col;
						$html			=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'select', $value, '', $choices );
						$html			=	$this->_fieldSearchModeHtml( $field, $user, $html, 'none' );		//TBD: Has avatarapproved...
					} else {
	...
	function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
		...
		switch ( $choice ) {
			case 'upload':
				...
				$imgToolBox->_maxsize			=	$field->params->get( 'avatarSize', $ueConfig['avatarSize'] );
				$imgToolBox->_maxwidth			=	$field->params->get( 'avatarWidth', $ueConfig['avatarWidth'] );
				$imgToolBox->_maxheight		=	$field->params->get( 'avatarHeight', $ueConfig['avatarHeight'] );
				$imgToolBox->_thumbwidth		=	$field->params->get( 'thumbWidth', $ueConfig['thumbWidth'] );
				$imgToolBox->_thumbheight		=	$field->params->get( 'thumbHeight', $ueConfig['thumbHeight'] );
				$imgToolBox->_debug				=	0;
...
*/
			...
			case 'delete':
				...
				break;
			case 'approve':
				if ( ( $_CB_framework->getUi() == 2 ) && $user->id && $user->$col != null && $user->$colapproved == 0 ) {
					$user->$colapproved			=	1;
		...
		return true;				// nothing to do, Image fields don't save for now :-(
	}
	/**
	 * Finder:
	 * Prepares field data for saving to database (safe transfer from $postdata to $user)
	...
	function _avatarHtml( &$field, &$user, $reason, $thumbnail = true, $show_avatar = 2 ) {
		...
		$return							=	$aTag
										.	'<img src="' . $imgUrl . '" alt="' . $name . '" title="' . $name . '" class="' . $class . '" />'
										.	$naTag;
		return $return;
...
*/
	...
	function _htmlEditForm( &$field, &$user, $reason, $displayFieldIcons = true ) {
		...
		$choices						=	array();
		if ( ( $reason == 'register' ) || ( ( $reason == 'profile' ) && ( $user->id == 0 ) ) ) {
			if ( $field->required == 0 ) {
...
*/
		...
		if ( ( $name != 'avatar' ) || $ueConfig['allowAvatarUpload'] ) {
			...
			$html	.=	'<div id="cbimg_upload_' . $name . '">'
					.		'<p>' . sprintf( _UE_UPLOAD_DIMENSIONS_AVATAR, $field->params->get( 'avatarWidth', $ueConfig['avatarWidth'] ), $field->params->get( 'avatarHeight', $ueConfig['avatarHeight'] ), $field->params->get( 'avatarSize', $ueConfig['avatarSize'] ) ) . '</p>'
					.		'<div>' . _UE_UPLOAD_SELECT_FILE . ' '
					.			'<input type="file" name="' . $name . '__file" value="" />'
					.		'</div>'
		...
		if ( ( $ueConfig['allow_onlinestatus'] == 1 ) && isset( $user ) && $user->id ) {
			...
			switch ( $output ) {
				...
				case 'htmledit':
					$oReturn			=	null;
					if ( $reason == 'search' ) {
						$oReturn		=	$this->_fieldSearchModeHtml( $field, $user, $oReturn, 'none' );		//TBD: is online or not...
					}
					break;
			...
			switch ( $output ) {
				...
				case 'htmledit':
					...
					if ( $reason == 'search' ) {
						...
						$oReturn			=	$this->_fieldSearchModeHtml( $field, $user, $html, 'isisnot' );
...
class CBfield_formatname extends cbFieldHandler {
	...
	function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
		...
		return true;				// nothing to do, Formatted names fields don't save :-)
	}
	/**
	 * Finder:
	 * Prepares field data for saving to database (safe transfer from $postdata to $user)
...
class CBfield_delimiter extends cbFieldHandler {
	...
	function prepareFieldDataSave( &$field, &$user, &$postdata, $reason ) {
		...
		return true;				// nothing to do, Delimiter fields don't save :-)
	}
	/**
	 * Finder:
	 * Prepares field data for saving to database (safe transfer from $postdata to $user)
	...
	function getFieldRow( &$field, &$user, $output, $formatting, $reason ) {
		...
		if( $_CB_framework->getUi() == 2 ) {
			...
			if (( $user_group == 'super administrator' && $myGid != 25) || ( $user->id == $_CB_framework->myId() && $myGid == 25)) {
				...
				$lists['gid']				=	moscomprofilerHTML::selectList( $gtree, 'gid', 'class="inputbox" size="11" mosReq="0"', 'value', 'text', $user->gid, 2 );
			}
			...
			if( $user->id) {
				...
				$paramField->_html			=	$user->registerDate;
				$paramField->description	=	'';
				...
				$paramField->_html			=	$user->lastvisitDate;
				$paramField->description	=	'';

...
function tabClass( $option, $task, $uid ) {
	...
	if ( $task == 'fieldclass' ) {
		...
		if ( $user && $user->id ) {
			...
			if ( $msg === null ) {
				// safeguard against missconfiguration of the above: also avoids lower-level users editing higher level ones:
				$msg		=	checkCBpermissions( array( (int) $user->id ), 'profile', true );
			}
			if ( $msg ) {
				echo $msg;
				return;
...
*/
	...
	for ( $i=0, $n=count( $plists ); $i < $n; $i++ ) {
		$plist				=&	$plists[$i];
	   	$publishedlists[]	=	moscomprofilerHTML::makeOption( $plist->listid, cbReplaceVars( $plist->title, $myUser ) );
	}
...
*/
	...
	$searchType				=	$params->get( 'list_compare_types', 0 );
	$searchVals				=	new stdClass();
	$searchesFromFields		=	$tabs->applySearchableContents( $searchableFields, $searchVals, $_GET /*, $searchType */ );
	$whereFields			=	$searchesFromFields->reduceSqlFormula( $tableReferences, TRUE );
	if ( $whereFields ) {
...
*/
	...
	HTML_comprofiler::usersList( $row, $users, $columns, $allFields, $lists, $listid, $search, $searchmode, $option_itemid, $limitstart, $limit, $total, $myUser, $searchableFields, $searchVals, $tabs, $searchType, $showPaging, $hotlink_protection );
}
/**
...
function registerForm( $option, $emailpass, $regErrorMSG = null ) {
	...
	$userComplete						=	new moscomprofilerUser( $_CB_database );
	HTML_comprofiler::registerForm( $option, $emailpass, $userComplete, $regErrorMSG );
}
	...
	if ( $_CB_database->loadObject( $dataObj ) ) {
		if ( $dataObj->result ) {
			...
			if ( $function == 'testexists' ) {
				echo ( '<span class="cb_result_ok">' . sprintf( _UE_USERNAME_EXISTS_ON_SITE, htmlspecialchars( $username ) ) . '</span>' );
			} else {
				echo ( '<span class="cb_result_error">' . sprintf( _UE_USERNAME_ALREADY_EXISTS, htmlspecialchars( $username ) ) . '</span>' );
			}
		} else {
			if ( $function == 'testexists' ) {
				echo ( '<span class="cb_result_error">' . sprintf( _UE_USERNAME_DOES_NOT_EXISTS_ON_SITE, htmlspecialchars( $username ) ) . '</span>' );
			} else {
				echo ( '<span class="cb_result_ok">' . sprintf( _UE_USERNAME_DOESNT_EXISTS, htmlspecialchars( $username ) ) . '</span>' );
			}
		}
	} else {
		echo ( '<span class="cb_result_error">' . _UE_SEARCH_ERROR . ' !' . '</span>' );
	}
}
...
function performCheckEmail( $email, $function ) {
	...
	if ( $ueConfig['reg_email_checker'] > 1 ) {
		...
		if ( $_CB_database->loadObject( $dataObj ) ) {
			if ( $function == 'testexists' ) {
				if ( $dataObj->result ) {
					echo '<span class="cb_result_ok">' . sprintf( _UE_EMAIL_EXISTS_ON_SITE, htmlspecialchars( $email ) ) . "</span>";
					return;
				} else {
					echo '<span class="cb_result_error">' . sprintf( _UE_EMAIL_DOES_NOT_EXISTS_ON_SITE, htmlspecialchars( $email ) ) . "</span>";
					return;
				}
			} else {
				if ( $dataObj->result ) {
					echo '<span class="cb_result_error">' . sprintf( _UE_EMAIL_ALREADY_REGISTERED, htmlspecialchars( $email ) ) . "</span>";
					return;
				}
			}
		}
	...
	switch ( $checkResult ) {
		case -2:
			echo '<span class="cb_result_error">' . sprintf( _UE_EMAIL_NOVALID, htmlspecialchars( $email ) ) . "</span>";
			break;
		case -1:
			echo '<span class="cb_result_warning">' . sprintf( _UE_EMAIL_COULD_NOT_CHECK, htmlspecialchars( $email ) ) . "</span>";
			break;
		case 0:
			if ( $ueConfig['reg_confirmation'] == 0 ) {
				echo '<span class="cb_result_error">' . sprintf( _UE_EMAIL_INCORRECT_CHECK, htmlspecialchars( $email ) ) . "</span>";
			} else {
				echo '<span class="cb_result_error">' . sprintf( _UE_EMAIL_INCORRECT_CHECK_NEEDED, htmlspecialchars( $email ) ) . "</span>";
			}
			break;
		case 1:
			echo '<span class="cb_result_ok">' . sprintf( _UE_EMAIL_VERIFIED, htmlspecialchars( $email ) ) . "</span>";
			break;
		default:
...
function login( $username=null, $passwd2=null ) {
	...
	if ( !$username || !$passwd2 ) {
		...
		if ( $resultError ) {
			...
			if ( $return && !( strpos( $return, 'comprofiler' /* 'com_comprofiler' */ ) && ( strpos( $return, 'login') || strpos( $return, 'registers' ) || strpos( strtolower( $return ), 'lostpassword' ) ) ) ) {
			// checks for the presence of a return url
			// and ensures that this url is not the registration or login pages
				$returnURL = (strncasecmp($return, "http:", 5)||strncasecmp($return, "https:", 6)) ? $return : cbSef( $return, false );
			} elseif ( ! $returnURL ) {
...
function approveUser($uids) {
	...
	foreach($uids AS $uid) {
		...
		if ( $ueConfig['emailpass'] == "1" ) {
			...
			$row->password	=	$pwd;
			$pwd			=	", password = " . $_CB_database->Quote( $pwd_md5 ) . " ";
		} else {
			$pwd			=	'';
		}

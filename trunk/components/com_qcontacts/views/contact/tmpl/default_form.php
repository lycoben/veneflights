...
<!--
	function validateForm( frm ) {
		...
		if(!regex.test(frm.email.value)) {
			...
			return false;
		} else if (frm.body.value == ''<?php foreach($this->fields as $f) {$fld=$f['field'];if($fld->required){echo ' || frm.'. $fld->name . '.value ==\'\'';} } ?>) {
...
<form action="<?php echo JRoute::_( 'index.php?option=com_qcontacts&view=contact&id='.$this->contact->slug.'&catid='.$this->contact->catslug );?>" method="post" name="emailForm" id="emailForm" class="form-validate" onsubmit="return validateForm(this);">
	<div class="contact_email<?php echo $this->params->get('pageclass_sfx'); ?>">
		...
		switch($fld->type) {
			case 'text':
			?>
			<input type="text" name="<?php echo $n;?>" id="<?php echo $fld->id;?>" size="<?php echo $fld->size;?>" value="<?php echo $this->data->$n ?>" class="inputbox" />
			<?php
			break;
			case 'textarea':
			?>
			...
			case 'radio':
				...
				foreach($fld->value as $r) {
					?>
					<input type="radio" name="<?php echo $n;?>" class="radio" id="<?php echo $fld->id;?>" value="<?php echo $r; ?>"<?php echo ($this->data->$n == $r ? ' checked="checked"':''); ?> /><?php echo $r; ?> 
					<?php
				}
				?></div><?php
				break;
			case 'checkbox':
				?>
				<div class="fld-wrap"><input type="checkbox" name="<?php echo $n;?>" class="chkbox" id="<?php echo $fld->id;?>" value="<?php echo $fld->value; ?>"<?php echo ($this->data->$n == $fld->value ? ' checked="checked"':''); ?> />
			...
			case 'dropdown':
				...
				foreach($fld->value as $r) {
					?>
					<option value="<?php echo $r;?>"<?php echo($this->data->$n == $r ? ' selected="selected"':''); ?>><?php echo $r;?></option>
					<?php
				}
				?></select><?php
				break;

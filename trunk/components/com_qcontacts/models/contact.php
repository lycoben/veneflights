...
class QContactsModelContact extends JModel {
	...
	function mailTo() {
		...
		if (!$this->_data->email || !$this->_data->body || (JMailHelper::isEmailAddress($this->_data->email) == false))
		{
		...
		if (!$pparams->get('custom_reply')) {
			...
			$body 	= $prefix."\n".$this->_data->name.' <'.$this->_data->email.'>'."\r\n\r\n".stripslashes($this->_data->body);
			
			for($i=1; $i<=$this->max_cust_fields; $i++){
				$custs = $cparams->get('cust'.$i.'_show');
				if($custs == '') {
					$custs = $pparams->get('cust'.$i.'_show');
				}
				if($custs) {
					$cust = $cparams->get('cust'.$i.'_label');
					if(!$cust) {
			...
			if ( $this->_data->email_copy && $emailcopyCheck )
			{

	...
	function selectList( &$arr, $tag_name, $tag_attribs, $key, $text, $selected, $required = 0 ) {
		...
		for ( $i = 0, $n = count( $arr ); $i < $n; $i++ ) {
			...
			if ( is_array( $arr[$i]->$key ) ) {
				...
				$html				.=	/* &nbsp; of userGroups: cannot do this: htmlspecialchars( */ getLangDefinition( $t );
				$html				.=	"</option>";
 ...
 *
 * @param  moscomprofilerUser  $user  object mos+cbUser
...
*/
		...
		DEFAULT:
			if ( $field != null ) {
				$args		=	array( &$field, &$user, 'html', 'view' );
...
function outputCbTemplate( $ui, $templateFile = 'template.css', $media = null ) {
	addCbHeadTag( $ui, '<link type="text/css" rel="stylesheet" href="' . selectTemplate($ui) . $templateFile . '"' . ( $media ? ' media="' . $media . '"' : '' ) . ' />' );
}
/**
* Outputs an arbitrary html text into head tags if possible and configured, otherwise echo it.
...
function outputCbJs($ui) {
	global $_CB_framework;
	addCbHeadTag( $ui, '<script type="text/javascript" src="' . $_CB_framework->getCfg('live_site') . '/components/com_comprofiler/js/cb12' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . '.js"></script>' );
}
...
function initToolTip( $ui, $width='250' ) {
	...
	if ( ! $outputed ) {
		global $_CB_framework;
		$live			=	$_CB_framework->getCfg( 'live_site' );
		$tip= '<script type="text/javascript" src="' . $live . '/components/com_comprofiler/js/overlib_mini.js"></script>'
		."\n".'<script type="text/javascript" src="' . $live . '/components/com_comprofiler/js/overlib_hideform_mini.js"></script>'
		."\n".'<script type="text/javascript" src="' . $live . '/components/com_comprofiler/js/overlib_anchor_mini.js"></script>'
		."\n".'<script type="text/javascript" src="' . $live . '/components/com_comprofiler/js/overlib_centerpopup_mini.js"></script>'
...
function CB45_mosToolTip( $ui, $tooltip, $title='', $width='', $image='tooltip.png', $htmltext='', $href='', $style='', $olparams='',$click=false, $altText=' ' ) {
	if ( $altText == ' ' ) {
		$altText	=	' alt="' . strip_tags( sprintf( _UE_INFORMATION_FOR_FIELD, stripslashes( $title ), stripslashes( $tooltip ) ) ) . '"';
	}
	if ( $width ) {
		$width = ', WIDTH, \''.$width .'\'';
	}
...
function getFieldIcons($ui, $oReq, $oProfile, $oDescription="", $oTitle="", $showLabels=false) {
	...
	//echo "templatePath=<pre>";print_r($templatePath);echo "</pre><hr>";
...
function cbJumpOutOfIframe() {
	return '<script type="text/javascript">if(self!=top)top.location=self.location;</script>';
}
...
function cbGetRegAntiSpamInputTag( $cbGetRegAntiSpams = null ) {
	...
	setcookie( cbGetRegAntiSpamCookieName( $cbGetRegAntiSpams[0] ), $cbGetRegAntiSpams[1], false, '/' );
...
function cbRegAntiSpamCheck( $mode = 1 ) {
	...
	if ( $validateCookieName === false ) {
		...
		$validateValueCookie	=	cbGetParam( $_COOKIE, $validateCookieName );
	...
	//		break;
			...
			CASE 'editorta':
				if(!($oReadOnly > 0)) {
					...
					$oReturn	.=	' <script type="text/javascript">document.adminForm.'.$oName.".setAttribute('mosReq',".$oReq."); document.adminForm.".$oName.".setAttribute('mosLabel','". addslashes( getLangDefinition($oLabel) ) ."'); </script>";
				} else {
					$oReturn = $oValue;
				}
			break;
	...
	*/
	function cbAddCalendar( $oName, $oLabel, $oReq, $oValue = "", $oReadOnly = false, $showTime = false, $yMin = null, $yMax = null ) {
		...
		if ( $this->calendarType == 1 ) {
			...
			$return = '<input class="inputbox"'.$vardisabled.' mosReq="'.$oReq.'" mosLabel="'. htmlspecialchars( getLangDefinition($oLabel) ) .'"'
			...
			if ( $oReadOnly ) {
				...
				$return			=	"<script type=\"text/javascript\">cbcalDateInput('" . $oIdName . "', " . $jsReqBoolean . ", '" . $this->dateFormat . "', '" . $oValue . "', '" . $oName . "', '" . $AdditionalInputAttributes . "'" . $years . ")</script>";
			}
		}
		return $return;
	}
	...
	function _addHeadTag() {
		...
		if ( ! $added[$this->calendarType] ) {
			...
			if ( file_exists( $UElanguagePath.'/'.$_CB_framework->getCfg( 'lang' ).'/calendar-locals.js' ) ) {
				$calendarLangFile	=	$_CB_framework->getCfg( 'live_site' ).'/components/com_comprofiler/plugin/language/'.$_CB_framework->getCfg( 'lang' ).'/calendar-locals.js';
			} else {
				$calendarLangFile	=	$_CB_framework->getCfg( 'live_site' ).'/components/com_comprofiler/plugin/language/default_language/calendar-locals.js';
			}

			if ( $this->calendarType == 1 ) {
				addCbHeadTag( $this->ui, '<link rel="stylesheet" type="text/css" media="all" href="'.selectTemplate($ui).'calendar.css" title="win2k-cold-1" />'
				."\n".'<script type="text/javascript" src="'. $_CB_framework->getCfg( 'live_site' ) . '/components/com_comprofiler/js/calendar' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . '.js"></script>'."\n"
				.'<script type="text/javascript" src="'. $calendarLangFile . '"></script>'."\n"		// language
				.'<script type="text/javascript" src="'. $_CB_framework->getCfg( 'live_site' ) . '/components/com_comprofiler/js/calendar-setup' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . '.js"></script>' );
			} else {
				addCbHeadTag( $this->ui,
					  '<script type="text/javascript">var cbTemplateDir="' . selectTemplate( $this->ui ) . '"; Calendar = function () { };</script>'."\n"
					. '<script type="text/javascript" src="'. $calendarLangFile . '"></script>'."\n"		// language
					. '<script type="text/javascript" src="'. $_CB_framework->getCfg( 'live_site' ) . '/components/com_comprofiler/js/calendardateinput' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . '.js"></script>' );
			}
...
class cbTabs extends cbTabHandler {
	...
	var $_stepDownFormatting	=	array( 'table' => 'tr', 'tabletrs' => 'tr', 'tr' => 'td', 'td' => 'div', 'divs' => 'div', 'div' => 'span', 'span' => 'span', 'none' => 'none' );
	...
	*/
	function cbTabs( $useCookies, $ui, $unusedNowOldCalendars = null, $outputTabpaneScript = true ) {
		global $_CB_framework;
		...
		if ( $outputTabpaneScript ) {
			addCbHeadTag( $ui, '<script type="text/javascript" src="' . $_CB_framework->getCfg( 'live_site' ) . '/components/com_comprofiler/js/tabpane' . ( $_CB_framework->getCfg( 'debug' ) ? '' : '.pack' ) . '.js"></script>' );
		}
	}
	...
	*/	
	function _loadTabsList( &$user, $position = '' ) {
		...
		if ( ! isset( $this->tabsToDisplay[$position] ) ) {
			...
			$this->tabsToDisplay[$position]		=	$_CB_database->loadObjectList( null, 'moscomprofilerTabs', array( &$_CB_database ) );
		}
	}
	/**
	...
	*/	
	function getViewTabs( $user, $position = '' ) {
		...
		foreach( $this->tabsToDisplay[$position] AS $k => $oTab ) {
			...
			if ( $oTab->fields ) {
				...
				$oContent[$k]					.=	$this->_getTabContents( $oTab->tabid, $user, $this->tabsToDisplay[$position][$k]->_fieldsCount );
				$tabOneTwoRowsStyleToggle[$pos]	=	$_CB_OneTwoRowsStyleToggle;
			}
		}
		...
		foreach( $this->tabsToDisplay[$position] AS $k => $oTab ) {
			...
			if ( ( $oContent[$k] != '' ) || ( $oTab->fields && ( $oTab->_fieldsCount > 0 ) && isset( $ueConfig['showEmptyTabs'] ) && ( $ueConfig['showEmptyTabs'] == 1 ) ) ) {
				...
				switch ($oTab->displaytype) {
					...
					case "overlib":
						...
						$fieldTip	= "&lt;div class=\"contentpaneopen cb_tab_content cb_tab_overlib\" id=\"cb_tabid_" . $oTab->tabid . "\" style=\"width:100%\"&gt;".$oContent[$k]."&lt;/div&gt;";
					...
					case "overlibfix":
						$tipTitle	= $htmltext = $tabTitle;
						$fieldTip	= "&lt;div class=\"contentpaneopen cb_tab_content cb_tab_overlib_fix\" id=\"cb_tabid_" . $oTab->tabid . "\" style=\"width:100%\"&gt;".$oContent[$k]."&lt;/div&gt;";
						$style		= "class=\"cb-tips-hover\"";
					...
					case "overlibsticky":
						...
						$fieldTip	=	"&lt;div class=\"contentpaneopen cb_tab_content cb_tab_overlib_sticky\" id=\"cb_tabid_" . $oTab->tabid . "\" style=\"width:100%\"&gt;".$oContent[$k]."&lt;/div&gt;";
	...
	function getEditTabs( &$user, $postdata = null, $formatting = 'table', $reason = 'profile', $tabbed = true ) {
		...
		$initFieldsToDefault				=	( ( $reason == 'register' ) && ( $postdata === null ) )
												|| ( ( $reason == 'profile' ) && ( $user->id == null ) && ( $postdata === null ) );
		...
		foreach( $oTabs AS $oTab ) {
			...
			if ( $oTab->fields ) {
				...
				$oContent					.=	$this->_getEditTabContents( $oTab, $user, $formatting, $reason, true );
			}
	...
	function _getTabNavJS( $pID, &$tabs ) {
		...
		if ( ! $functionOutputed ) {
			$head		=	"<script type=\"text/javascript\"><!--//--><![CDATA[//><!--\n"
						...
						.	"}\n"
						.	"//--><!]]></script>\n";
			addCbHeadTag( $_CB_framework->getUi(), $head );
	...
	function & _getTabsDb( &$user, $reason ) {
		...
		if ( $tabsCache === null ) {
			...
			$_CB_database->setQuery( $sql );
			$tabsCache		=	$_CB_database->loadObjectList( null, 'moscomprofilerTabs', array( &$_CB_database ) );
		}
		return $tabsCache;
	}
	function _getTabFieldsDb( $tabid, &$user, $reason, $fieldName = null, $prefetchFields = true ) {
		...
		if ( ( ! $prefetchFields ) || ( $prefetched === null ) ) {
			...
			if ( $fieldName ) {
				$where[]		=	'f.name = ' . $_CB_database->Quote( $fieldName );
			}
			$where[]			=	'f.published = 1';
			if ( $reason == 'view' ) {
				$where[]		=	'f.profile != 0';
			...
			if ( ( $reason == 'view' ) && ( $ueConfig['allow_email_display'] == 0 ) ) {
			...
			if ( is_array( $fields ) ) {
				if ( $prefetchFields ) {
					...
					//	$prefetched[(int) $fields[$i]->tabid][$i]				=	$fields[$i];
						$prefetched[(int) $fields[$i]->tabid][$fields[$i]->fieldid]				=	$fields[$i];
					}
				} else {
					for ( $i = 0, $n = count( $fields ) ; $i < $n ; ++$i ) {
						$fields[$i]->params										=	new cbParamsBase( $fields[$i]->params );
		...
		global $_PLUGINS;
		$_PLUGINS->loadPluginGroup( 'user' );
		$_PLUGINS->trigger( 'onAfterFieldsFetch', array( &$fields, &$user, $reason, $tabid, $fieldName ) );
	...
	function _getViewTabContents( $tabid, &$user ) {
		$oFields				=	$this->_getTabFieldsDb( $tabid, $user, 'view' );
		return $this->_getFieldsContents( $oFields, $user, (int) $tabid );
	}

	function _getTabContents( $tabid, &$user, &$fieldsCount, $output = 'html', $formatting = 'table', $reason = 'profile' ) {
		$oFields				=	$this->_getTabFieldsDb( $tabid, $user, 'view' );
		$fieldsCount			=	count( $oFields );
		return $this->_getFieldsContents( $oFields, $user, (int) $tabid, $output, $formatting, $reason );
	}

	function _getEditTabContents( &$tab, &$user, $formatting = 'table', $reason = 'profile', $prefetchFields = true ) {
		$results				=	'';
		...
		if ( count( $fields ) > 0 ) {
			...
			$results			.=	$this->_getFieldsContents( $fields, $user, $tabid, 'htmledit', $formatting, $reason );
		}
		return $results;
	}
	function getSearchablesContents( &$searchableFields, &$userMe, &$searchVals, $output = 'htmledit', $formatting = 'divs', $reason = 'search' ) {
		global $_CB_framework, $_CB_database;
...
*/
			$results			=	$this->_getFieldsContents( $searchableFields, $user, 'listsearch', $output, $formatting, $reason );
		}
		return $results;
	}
	function applySearchableContents( &$searchableFields, &$searchVals, &$postdata, $reason = 'search' ) {
		global $_PLUGINS;
		...
		foreach ( $searchableFields as $field ) {
			$fieldSearches		=	$_PLUGINS->callField( $field->type, 'bindSearchCriteria', array( &$field, &$searchVals, &$postdata, $reason ), $field );
	...
	function savePluginTabs( &$user, &$postdata ) {
		...
		$oTabs					=	$this->_getTabsDb( $user, 'profile' );
	...
	function _getFieldsContents( &$oFields, &$user, $tabid, $output = 'html', $formatting = 'table', $reason = 'profile' ) {
		...
		if ( is_array( $oFields ) ) {
			...
			if ( cbStartOfStringMatch( $output, 'html' ) ) {
				...
				foreach( $oFields AS $oField ) {
					$results							.=	$this->_getSingleFieldContent( $oField, $user, $output, $formattingFields, $reason );
				}
	...
	function _getSingleFieldContent( &$oField, &$user, $output = 'html', $formatting = 'tr', $reason = 'profile' ) {
		global $_PLUGINS;
		return $_PLUGINS->callField( $oField->type, 'getFieldRow', array( &$oField, &$user, $output, $formatting, $reason ), $oField );
	}
	...
	*/	
	...
	function fieldCall( $fieldName, &$user, &$postdata ) {
		global $_CB_framework, $_PLUGINS;

		if ( ( $user === null ) && ( $_CB_framework->getUi() == 1 ) ) {
			$reason						=	'register';
		} else {
			$reason						=	'profile';
		}

		$fields							=	$this->_getTabFieldsDb( null, $user, $reason, $fieldName, false );
		if ( is_array( $fields ) && ( count( $fields ) == 1 ) ) {
			foreach ( $fields as $field ) {
				$_PLUGINS->loadPluginGroup( 'user', array( $field->pluginid ) );
...
class cbConnection {
	...
	function getConnectionsCount( $userid )  {
		global $_CB_database;
	...
	function getPendingConnections( $userid, $offset = 0, $limit = 500 ) {
		global $_CB_database;
	...
	function getActiveConnections( $userid, $offset = 0, $limit = 500 ) {
		global $_CB_database;
		$query = "SELECT DISTINCT m.*,u.name,u.email,u.username,c.avatar,c.avatarapproved, u.id, IF(s.session_id=null,0,1) AS 'isOnline' "
		. "\n FROM #__comprofiler_members AS m"
		. "\n LEFT JOIN #__comprofiler AS c ON m.memberid=c.id"
	...
	function getConnectedToMe( $userid, $offset = 0, $limit = 500 ) {
		global $_CB_database;
		$query = "SELECT DISTINCT m.*,u.name,u.email,u.username,c.avatar,c.avatarapproved, u.id, IF(s.session_id=null,0,1) AS 'isOnline' "
		. "\n FROM #__comprofiler_members AS m"
		. "\n LEFT JOIN #__comprofiler AS c ON m.referenceid=c.id"
...
class cbNotification {
	...
	function sendFromSystem( $toid, $sub, $message, $replaceVariables = true ) {
		...
		if ($replaceVariables) {
			$sub				=	$this->_replaceVariables( $sub, $rowTo );
		...
		return $this->_sendEmailMSG($rowTo,$rowFrom,cb_html_entity_decode_all($_CB_framework->getCfg( 'sitename' )).' - '.$sub,$message,FALSE);
	...
	function _sendEmailMSG( $to, $from, $sub, $msg, $addPrefix = false, $mode = 0 ) {			//BB: add html
...
//		if (class_exists("mosPHPMailer")) {
			$res			=	comprofilerMail( $from->email, $from->name, $to->email, $sub, $msg, $mode, NULL, NULL, NULL, $from->replytoEmail, $from->replytoName );
...
*/
	...
	function _replaceVariables($msg, $row){
		...
		$msg = cbReplaceVars( $msg, $row, false );		// this is for plaintext emails, no htmlspecialchars needed here.
		return $msg;
	}
...
class cbFields {
	...
	function prepareFieldDataSave( $fieldId, $fieldType, $fieldName, $value=null, $registration=0,$field=null ) {
		...
		default:
			...
			if ( $field != null ) {
				$args	=	array( &$field, &$user, &$_POST, 'profile' );

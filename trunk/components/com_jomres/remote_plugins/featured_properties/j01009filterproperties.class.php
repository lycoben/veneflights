...
class j01009filterproperties
	...
	function j01009filterproperties($componentArgs)
		...
		global $jomresAdminPath,$Itemid,$jomresConfig_live_site;
		...
		if (count($featured)>0)
			...
			foreach ($featured as $f)
				...
				if (in_array($f,$this->propertys_uids) )
					{
					$featured_property_ids[]=$f;
					}
				}
			if (count($featured_property_ids)>0)
				{
				$newArray=array();
				foreach ($this->propertys_uids as $id)
					{
					if (!in_array($id,$featured_property_ids))
						$newArray[]=$id;
					}

				$this->propertys_uids=array_merge($featured_property_ids,$newArray);
				}
			}

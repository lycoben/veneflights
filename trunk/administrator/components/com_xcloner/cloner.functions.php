   ...
   if(is_array($_COOKIES))
    ...
    #### END TAR MODE ############################################################
      ...
      #### CREATE BACKUP USING SERVER UTILITIES
       ...
       exec($_CONFIG[tarpath]." $exclude -c".$_CONFIG['tarcompress']."vf $filename ./backups/index.html");
       exec($_CONFIG[tarpath]." -".$_CONFIG['tarcompress']."vf $filename --update ./backups/database-sql.sql");
       exec($_CONFIG[tarpath]." -".$_CONFIG['tarcompress']."vf $filename --update ./backups/htaccess.txt");
       exec($_CONFIG[tarpath]." -".$_CONFIG['tarcompress']."vf $filename --update ./backups/perm.txt");
       ...
       if(file_exists($filename))
       {
        ...
        if($_REQUEST[cron_dbonly]!=1){
		 ...
		 exec($_CONFIG[tarpath]."  -".$_CONFIG['tarcompress']."cvf $filename ./backups/database-sql.sql");
         
         if(is_array($databases_incl)){
             foreach($databases_incl as $database_name)
				 if($database_name!="") {
	                exec($_CONFIG[tarpath]."  -".$_CONFIG['tarcompress']."vf $filename --update  ./backups/".$database_name."-sql.sql");
			     }
         }

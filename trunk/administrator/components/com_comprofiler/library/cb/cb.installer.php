...
class cbInstallerPlugin extends cbInstaller {
	...
	*/
	function install( $p_fromdir = null ) {
		...
		$version				=	$v->data();
		if (($version == $ueConfig['version']) || ( $version=="1.2 RC 2" || $version=="1.2 RC" || $version=="1.0 RC 2" || $version=="1.0" || $version=="1.0.1" || $version=="1.0.2" || $version=="1.1")) {
			;
		} else {
...
*/
	...
	*/
	function getXml( $id ) {
		...
		$this->i_elementdir			=	$_CB_framework->getCfg('absolute_path') . '/components/com_comprofiler/plugin/' . $row->type . '/'.$row->folder.'/';
		$this->i_installfilename	=	$this->i_elementdir . $row->element . '.xml';
		...
		cbimport('cb.xml.simplexml');
		return new CBSimpleXMLElement( trim( file_get_contents( $this->i_installfilename ) ) );
	}
	/**

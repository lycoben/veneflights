...
*/
	...
	function & getInstance( $userId ) {
		...
		return CBuser::_getOrSetInstance( $userId );
	}
	/**
	 * Vreates and sets a new instance of CBuser to $user
	 * @static
	 *
	 ...
	 * @return mixed                
	 */
	function getField( $fieldName, $defaultValue = null, $output = 'html', $formatting = 'none', $reason = 'profile' ) {
		...
		if ( isset( $fields[0] ) ) {
			$field		=	$fields[0];
			$value		=	$_PLUGINS->callField( $field->type, 'getFieldRow', array( &$field, &$this->_cbuser, $output, $formatting, $reason ), $field );
		} else {
			$value		=	$defaultValue;
		}
		return $value;		
...
//		$regex = '#\[indent]((?:[^[]|\[(?!/?indent])|(?R))+)\[/indent]#s';
	...
	function _evaluateCbTags( $input ) {
		...
		if ( is_array( $input ) ) {
			if ( isset( $input[3] ) ) {
				if ( $input[3] == '#displayed' ) {
					global $_CB_framework;
					$uid		=	$_CB_framework->displayedUser();
				} elseif ( preg_match( '/^[1-9][0-9]*$/', $input[3] ) ) {
					$uid		=	(int) $input[3];
				}
				if ( $uid ) {
...
class CBframework {
	...
	function CBframework( &$baseFramework, &$database, &$acl, &$aclParams, $cmsSefFunction, $sefFuncHtmlEnt, $cmsUserClassName, $cmsUserNeedsDb, $cmsRedirectFunction, $myId, $myUsername, $myUserType, $myCmsGid, $myLanguage, $cbUrlRouting, $getVarFunction,  $outputCharset, $editorDisplay ) {
	...
	function cbSef( $string, $htmlSpecials = true ) {
		if ( ( $this->getUi() == 1 )
			 && ( ( $string == '' ) || ( substr( $string, 0, 9 ) == 'index.php' ) || ( $string[0] == '?' ) )
...
$_CB_framework			=	new CBframework( $mainframe, $database, $acl, $aclParams, $sefFunc, $sefFuncHtmlEnt, $cmsUser, $cmsUserNeedsDb, $cmsRedirectFunc, $my->id, $my->username, $my->usertype, $myAid, $myLanguage, array( 'option' => $optionOfCb ), $getVarFunction, $outputCharst, $editorDisplay );

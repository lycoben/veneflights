 ...
 * @since 1.5
 */
class YOS_amMap_ControllerUpgrade extends YOS_amMap_Controller
{
	function doupdate(){
		global $mainframe;
		...
		if (!$autoupdate	=	& $engine->getInstance()) {
			$mainframe->redirect('index.php?option=com_yos_ammap&task=about');
			return false;
		}
		...
		$mainframe->redirect('index.php?option=com_yos_ammap&task=about', $autoupdate->getReport());
	}

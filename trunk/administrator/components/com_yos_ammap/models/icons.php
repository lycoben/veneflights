 ...
 * @since 1.5
 */
class YOS_ammap_ModelIcons extends JModel
{
	/**
	 * amMap data array
	 *
	...
	function __construct()
	{
		...
		$limit				= $mainframe->getUserStateFromRequest($option.'.icons.limit', 'limit', $mainframe->getCfg('list_limit'), 'int');
		$limitstart			= $mainframe->getUserStateFromRequest($option.'.icons.limitstart', 'limitstart', 0, 'int');
	...
	function getFilesFolder(){
		if (empty($this->_files)) {
			$this->_files 	=  JFolder::files(JPATH_SITE.DS.'components'.DS.'com_yos_ammap'.DS.'ammap'.DS.'icons',$this->getState('upload_ext'),0,false);
		}

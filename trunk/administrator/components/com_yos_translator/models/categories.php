 ...
 * @subpackage	Components
 */
class TranslatorModelCategories extends JModel
{
	/**
	 * modules data array
	 *
...
/**
	...
	function __construct()
	{
		...
		$filter_language	= $mainframe->getUserStateFromRequest($option.'.categories.filter_language', 'filter_language', '', 'string');
		
		$filter_status		= $mainframe->getUserStateFromRequest($option.'.categories.filter_status', 'filter_status', -1, 'int');
	...
	function _buildQuery($total = false){
		...
		if (!$total) {
			...
			(SELECT md5(JMD1.title) = JMC1.original_value FROM `#__categories` AS JMD1
		...
		$query .= "
			FROM `#__categories` AS JMD
			LEFT JOIN `#__jf_content` AS JFC ON JMD.id = JFC.reference_id AND JFC.reference_field = 'title' AND JFC.reference_table = 'categories' $leftJoinLanguage";

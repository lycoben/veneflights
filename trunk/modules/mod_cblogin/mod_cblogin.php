...
* @copyright (C) 2005-2008 Beat & JoomlaJoe & parts 2000 - 2005 Miro International Pty Ltd
...
if ( $_CB_framework->myId() ) {
		...
		if($pms != 0)
		{
			switch ($pms) {
				...
				case 2:
					...
					break;
				case 3:
					// $query_pms_count="SELECT count(id) FROM #__uddeim WHERE toread<1 AND toid=" . (int) $_CB_framework->myId();
					$query_pms_count="SELECT count(u.id) FROM #__uddeim u INNER JOIN #__users s ON u.fromid=s.id WHERE toread<1 AND toid=" . (int) $_CB_framework->myId();
					$_CB_database->setQuery($query_pms_count);
					$total_pms = $_CB_database->loadResult();	
	...
	switch ( $show_buttons_icons ) {
		case 2:
			$buttonStyle	=	' style="width:25px;height:20px;border-width:0px;margin:0px;cursor:pointer;vertical-align:top;background:transparent url(' . $urlImgPath . 'logout.png) no-repeat;"'
							.	' title="' . _UE_BUTTON_LOGOUT . '"';
			$buttonValue	=	'';
			break;
		case 1:
			$buttonStyle	=	' style="min-height:20px;padding-left:30px;cursor:pointer;background:transparent url(' . $urlImgPath . 'logout.png) no-repeat;"';
